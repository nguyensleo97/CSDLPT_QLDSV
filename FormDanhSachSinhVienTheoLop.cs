...
namespace QLDSV
{
    public partial class FormDanhSachSinhVienTheoLop : Form
    {
        private DataTable dt_DSSV;

        public FormDanhSachSinhVienTheoLop()
        {
            InitializeComponent();
        }
        ...
        private void FormDanhSachSinhVienTheoLop_Load(object sender, EventArgs e)
        {
        ...
        {
            ReportInDanhSachSinhVienTheoLop obj = new ReportInDanhSachSinhVienTheoLop();

            string lenh = "EXEC sp_INDANHSACHSINHVIENTHEOLOP '" + cmbLop.SelectedValue.ToString().Trim() + "'";

            DataTable dataTable = Program.ExecSqlDataTable(lenh);
            obj.SetDataSource(dataTable);
            obj.SetParameterValue("TENLOP", cmbLop.Text);

            crvDanhSachSinhVien.ReportSource = obj;
        }
